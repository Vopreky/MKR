	...
	for(Mesh::iteratorNode inode = m.BeginNode(); inode != m.EndNode(); inode++){
		...
		for (int k = 0; k < m; k++){
			...
			for (int k1 = 0; k1 < m1; k1++){
				...
				A[i][j] += d11 * area(c) * dx_basis_func(c, n, cen[0], cen[1]) * dx_basis_func(c, nj, cen[0], cen[1]);
				A[i][j] += d12 * area(c) * dx_basis_func(c, n, cen[0], cen[1]) * dy_basis_func(c, nj, cen[0], cen[1]);
				A[i][j] += d22 * area(c) * dy_basis_func(c, n, cen[0], cen[1]) * dy_basis_func(c, nj, cen[0], cen[1]);
			}
			areaSum += area(c);
